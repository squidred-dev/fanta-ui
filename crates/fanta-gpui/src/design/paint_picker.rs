...
impl PaintPicker {
    ...
    fn handle_resource_scope_menu_key(
        ...
        match event.keystroke.key.as_str() {
            ...
            "enter" | "space" => {
                self.select_resource_scope(
                    scopes[self.resource_scope_menu_index].clone(),
                    window,
                    cx,
                );
            }
    ...
    fn handle_creation_menu_key(
        ...
        match event.keystroke.key.as_str() {
            ...
            "enter" | "space" => {
                let kind = PaintCreationKind::ALL[self.creation_menu_index];
    ...
    fn track_bounds(&self, control: ContinuousControl, cx: &mut Context<Self>) -> AnyElement {
        let picker = cx.entity().downgrade();
        canvas(
            move |bounds, _, cx| {
                let _ = picker.update(cx, |this, _| match control {
                    ContinuousControl::ColorArea => this.color_area_bounds = bounds,
                    ContinuousControl::Hue => this.hue_bounds = bounds,
                    ContinuousControl::Alpha => this.alpha_bounds = bounds,
                });
            },
            |_, _, _, _| {},
        )
    ...
    fn render_color_area(&self, color: DesignColor, cx: &mut Context<Self>) -> AnyElement {
        ...
        div()
            ...
            .child(self.track_bounds(ContinuousControl::ColorArea, cx))
    ...
    fn render_hue_control(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        div()
            ...
            .child(self.track_bounds(ContinuousControl::Hue, cx))
    ...
    fn render_alpha_control(&self, color: DesignColor, cx: &mut Context<Self>) -> AnyElement {
        ...
        div()
            ...
            .child(self.track_bounds(ContinuousControl::Alpha, cx))
            .when(disabled, |control| control.opacity(0.56))
            .into_any_element()
    }
    ...
    fn render_color_style_sample_swatch(
        ...
        .on_key_down(cx.listener(move |this, event: &KeyDownEvent, window, cx| {
            if matches!(event.keystroke.key.as_str(), "enter" | "space") {
                this.request_color_style_sample(selection_for_key.clone(), cx);
                window.prevent_default();
                cx.stop_propagation();
            }
        }))
    ...
    fn render_color_variable_swatch(
        ...
        let color = paint_variable_color(variable);
        let variable_id = variable.id.clone();
        let variable_id_for_key = variable_id.clone();
        Button::new(SharedString::from(format!(
        ...
        .on_key_down(cx.listener(move |this, event: &KeyDownEvent, window, cx| {
            if matches!(event.keystroke.key.as_str(), "enter" | "space") {
                this.request_color_variable_apply(variable_id_for_key.clone(), cx);
                window.prevent_default();
                cx.stop_propagation();
            }
        }))
    ...
    fn render_picker(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let body = if self.active_tab == PaintPickerTab::Libraries {
            v_flex()
                ...
                .w_full()
                .max_h(px(PICKER_MAX_HEIGHT - PICKER_HEADER_HEIGHT))
                .overflow_y_scroll()
