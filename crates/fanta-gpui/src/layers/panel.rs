...
use gpui::{
    ...
    Focusable, InteractiveElement as _, IntoElement, KeyUpEvent, MouseButton, MouseDownEvent,
...
use super::{
    ActivateLayersControl, CloseLayersOverlay, LAYERS_CONTROL_KEY_CONTEXT,
    LAYERS_PANEL_KEY_CONTEXT, LayersPanelAction, LayersPanelContextAction, LayersPanelDropPosition,
...
const APPROXIMATE_LAYER_CHARACTER_WIDTH: f32 = 7.;
...
impl LayersPanel {
    ...
    fn render_header(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        h_flex()
            ...
            .on_action(cx.listener(|this, _: &ActivateLayersControl, _, cx| {
            ...
            .child(
                h_flex()
                    .id(SharedString::from(format!("{}-collapse-all", self.id)))
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .when(self.panel_expanded && self.menu.is_some(), |panel| {
                panel.child(self.render_context_menu(cx))
            })
    }
}
...
fn layer_title_min_width(title: &str, depth: usize) -> Pixels {
    px(
        title.chars().count() as f32 * APPROXIMATE_LAYER_CHARACTER_WIDTH
            + depth as f32 * LAYER_INDENT
            + 96.,
    )
}
