...
use crate::design::{DesignColor, DesignColorContrastCategory, DesignColorContrastLevel};
...
pub(super) fn format_decimal(value: f32) -> String {
    crate::design::format::format_compact_number(value)
}
...
impl PaintPicker {
    pub(super) fn set_color_format(
        ...
        self.forget_nested_overlay(PaintPickerOverlay::ColorFormat);
        // Changing formats can remove the field that opened the menu.
        // Keep keyboard focus on the retained picker instead of a detached input.
        InspectorOverlayFocusTarget::new(self.focus_handle.clone()).restore(window, cx);
    ...
    pub(super) fn preview_color_text_input(
        ...
        if !paint.kind.is_gradient() {
            let opacity = if parsed.explicit_alpha {
                f32::from(parsed.color.alpha) / 255. * 100.
            } else {
                self.hex_original_opacity
                    .unwrap_or_else(|| picker_opacity(paint, self.selected_stop))
            };
            let _ = self.emit_edit(
    ...
    pub(super) fn update_from_pointer(
        ...
        match control {
            ContinuousControl::ColorArea => {
                let x = fraction_x(self.color_area_bounds.inset(px(SELECTOR_INSET)), position);
                let y = fraction_y(self.color_area_bounds.inset(px(SELECTOR_INSET)), position);
                self.color_area_position = Some((x, 1. - y));
                self.set_saturation_value(x, 1. - y, cx);
            }
            ContinuousControl::Hue => {
                self.set_hue(
                    fraction_x(self.hue_bounds.inset(px(SELECTOR_INSET)), position) * 360.,
                    cx,
                );
            }
            ContinuousControl::Alpha => {
                self.set_alpha(
                    fraction_x(self.alpha_bounds.inset(px(SELECTOR_INSET)), position),
                    cx,
                );
            }
        }
    ...
    pub(super) fn handle_hex_input(
        ...
        match event {
            InputEvent::Change => {
                ...
                {
                    self.hex_original_opacity = self.current_picker_opacity();
    ...
    pub(super) fn render_color_area(
        ...
        let (selector_saturation, selector_value) = self
            .color_area_position
            .unwrap_or((hsv.saturation, hsv.value));
        ...
        div()
            .id(id)
            .debug_selector(|| "color-picker-spectrum".to_owned())
            .relative()
            .tab_index(0)
            .w_full()
            ...
            .on_drag(DragColorArea(entity_id), |drag, _, _, cx| {
                cx.stop_propagation();
                cx.new(|_| drag.clone())
            })
            .on_drag_move(
                cx.listener(move |this, event: &DragMoveEvent<DragColorArea>, _, cx| {
                    if event.drag(cx).0 == entity_id {
                        this.update_from_pointer(
                            ContinuousControl::ColorArea,
            ...
            .child(
                div()
                    .absolute()
                    .top(px(tokens::Space::XS / 4.))
                    .bottom(px(tokens::Space::XS / 4.))
                    .left(px(tokens::Space::XS / 4.))
                    .right(px(tokens::Space::XS / 4.))
                    .overflow_hidden()
                    .rounded(px(7. - tokens::Space::XS / 4.))
            ...
            .child(
                div()
                    .absolute()
                    .left(px(SELECTOR_INSET))
                    .right(px(SELECTOR_INSET))
                    .top(px(SELECTOR_INSET))
                    .bottom(px(SELECTOR_INSET))
                    .child(
                        crate::atoms::SliderHandle {
                            variant: crate::atoms::SliderHandleVariant::Chit,
                            state: if disabled {
                                crate::atoms::SliderState::Disabled
                            } else {
                                crate::atoms::SliderState::Default
                            },
                            color: Some(color_to_hsla(color)),
                        }
                        .render(cx)
                        .debug_selector(|| "color-picker-spectrum-thumb".to_owned())
                        .absolute()
                        .left(relative(selector_saturation))
                        .top(relative(1. - selector_value))
                        .ml(px(-tokens::SliderGeometry::HANDLE_SIZE / 2.))
                        .mt(px(-tokens::SliderGeometry::HANDLE_SIZE / 2.)),
                    ),
            )
    ...
    pub(super) fn handle_slider_action(
        &mut self,
        control: ContinuousControl,
        action: &crate::molecules::SliderAction,
        cx: &mut Context<Self>,
    ) {
        use crate::molecules::SliderPhase;
        match action.phase {
            SliderPhase::Begin => {
                let edit = match control {
                    ContinuousControl::Alpha => self.current_opacity_text_edit(),
                    _ => self.current_color_text_edit(),
                };
                if let Some(edit) = edit {
                    self.begin_continuous_edit(edit, cx);
                }
            }
            SliderPhase::Preview | SliderPhase::Commit => {
                match control {
                    ContinuousControl::Alpha => self.set_alpha(action.value, cx),
                    _ => self.set_hue(action.value * 360., cx),
                }
            ...
            SliderPhase::Cancel => {
                if let (Some(continuous), Some(target)) =
                    (self.continuous_edit.take(), self.target.clone())
                {
                    cx.emit(PaintPickerEvent::Edit {
                        target,
    ...
    pub(super) fn render_hue_control(&self, cx: &mut Context<Self>) -> AnyElement {
        self.hue_slider.update(cx, |slider, cx| {
            slider.set_value(self.remembered_hue / 360., cx);
            slider.configure(
                crate::molecules::SliderVariant::ColorRange,
                crate::atoms::SliderBackground::Hue,
                self.color_editing_disabled(),
                cx,
            );
        });
        self.hue_slider.clone().into_any_element()
    }
    ...
    pub(super) fn render_alpha_control(
        ...
        self.alpha_slider.update(cx, |slider, cx| {
            slider.set_value(self.current_picker_opacity().unwrap_or(100.) / 100., cx);
            slider.configure(
                crate::molecules::SliderVariant::Fill,
                crate::atoms::SliderBackground::Opacity(color_to_hsla(color)),
                self.opacity_editing_disabled(),
                cx,
            );
        });
        self.alpha_slider.clone().into_any_element()
    }

    pub(super) fn render_color_format_selector(&self, cx: &mut Context<Self>) -> AnyElement {
        let open = self.nested_overlay_is_open(PaintPickerOverlay::ColorFormat);
        let trigger =
            crate::atoms::ui_button(SharedString::from(format!("{}-color-format", self.id)))
                .debug_selector(|| "color-picker-format".to_owned())
                .label(self.color_format.label())
                .dropdown_caret(true)
        ...
        if open {
            ...
            control = control.child(div().absolute().top_0().left_0().child(
                crate::molecules::anchored_popup(
                    Anchor::BottomLeft,
                    point(gpui::Pixels::ZERO, px(-tokens::Space::XS)),
                    60,
                    menu,
                ),
            ));
    ...
    pub(super) fn render_color_channel_fields(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        for (channel, label) in labels.into_iter().enumerate() {
            ...
            fields = fields.child(
                div()
                    .debug_selector(move || format!("color-picker-channel-{channel}"))
                    .flex_1()
                    .min_w_0()
                    .child(
    ...
    pub(super) fn render_color_value_controls(&self, cx: &mut Context<Self>) -> AnyElement {
            ...
            .child(
                Input::new(&self.opacity_input)
                    ...
                    .when(self.opacity_invalid, |input| {
                        input.border_color(crate::atoms::SemanticColor::TextDanger.resolve(cx))
                    }),
            );
        ...
        if matches!(self.color_format, ColorFormat::Hex | ColorFormat::Css) {
            let controls = h_flex()
                ...
                .child(
                    div()
                        .debug_selector(|| "color-picker-hex".to_owned())
                        .flex_1()
                        .min_w_0()
                        .child(
