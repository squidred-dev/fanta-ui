...
use super::*;
use crate::atoms::ControlExt as _;
...
impl PaintPicker {
    ...
    pub(super) fn update_gradient_stop_from_pointer(
        ...
        self.selected_stop = index;
        let position = fraction_x(
            self.gradient_bounds
                .inset(px(tokens::SliderGeometry::STOP_SIZE / 2.)),
            position,
        );
        if let Some(edit) = self.gradient_stop_position_edit(position) {
    ...
    pub(super) fn render_gradient_preview(
        ...
        let mut preview = div()
            ...
            .debug_selector(|| "color-picker-gradient".to_owned())
            .relative()
            .tab_index(0)
            .w_full()
            .h(px(
                tokens::SliderGeometry::STOP_SIZE + tokens::SliderGeometry::TRACK_HEIGHT
            ))
            .on_mouse_down(
                MouseButton::Left,
        ...
        let background = crate::atoms::SliderBackground::Gradient(
            gradient_segments(paint)
                .into_iter()
                .flat_map(|(left, right, a, b)| {
                    [(left, color_to_hsla(a)), (right, color_to_hsla(b))]
                })
                .collect(),
        );
        preview = preview.child(
            div()
                ...
                .right_0()
                .bottom_0()
                .child(background.render(crate::atoms::SliderState::Default, cx)),
        );
        let mut stops = div()
            .absolute()
            .left(px(tokens::SliderGeometry::STOP_SIZE / 2.))
            .right(px(tokens::SliderGeometry::STOP_SIZE / 2.))
            .top_0()
            .bottom_0();
        for (index, stop) in paint.gradient_stops.iter().enumerate() {
            let selected = index == self.selected_stop;
            stops = stops.child(
                crate::atoms::SliderGradientStop {
                    color: color_to_hsla(stop.color),
                    selected,
                    disabled: self.editing_disabled(),
                }
                .render(
                    SharedString::from(format!("{}-gradient-handle-{index}", self.id)),
                    cx,
                )
                .debug_selector(move || format!("color-picker-gradient-stop-{index}"))
                .absolute()
                .left(relative(stop.position.clamp(0., 1.)))
                .ml(px(-tokens::SliderGeometry::STOP_SIZE / 2.))
                .top_0()
                .on_activate(cx.listener(move |this, _, window, cx| {
                    this.select_gradient_stop(index, window, cx)
                }))
                .on_key_down(cx.listener(move |this, event: &KeyDownEvent, window, cx| {
                    if this.editing_disabled() {
                        return;
                    }
                    this.select_gradient_stop(index, window, cx);
                    let position = this
                    ...
                    if let Some(edit) = this.gradient_stop_position_edit(next) {
                        let _ = this.emit_edit(edit, DesignPanelEditPhase::Commit, cx);
                    }
                    window.prevent_default();
                    cx.stop_propagation();
                }))
