...
#[gpui::test]
fn vector_eraser_toolbar_only_offers_size(cx: &mut TestAppContext) {
    let (host, cx) = setup(cx);
    let toolbar = cx.read(|app| host.read(app).toolbar.clone());
    cx.simulate_resize(size(px(1200.), px(800.)));
    cx.update(|_, app| {
        toolbar.update(app, |toolbar, cx| {
            toolbar.set_mode(ToolbarMode::Draw, cx);
            toolbar.set_active_tool(ToolbarTool::Eraser, cx);
            toolbar.set_draw_brush_capabilities(
                crate::toolbar::DrawBrushCapabilities::VECTOR_ERASER,
                cx,
            );
        })
    });
    cx.run_until_parked();
    assert!(cx.debug_bounds("toolbar-draw-size").is_some());
    for selector in [
        "toolbar-draw-choice-BrushTip",
        "toolbar-draw-hardness",
        "toolbar-draw-opacity",
        "toolbar-draw-flow",
        "toolbar-draw-smooth",
    ] {
        assert!(
...
fn host_draw_action_filter_hides_unsupported_path_actions(cx: &mut TestAppContext) {
    ...
    cx.update(|_, app| {
        toolbar.update(app, |toolbar, cx| {
            toolbar.set_mode(ToolbarMode::Draw, cx);
            toolbar.set_active_tool(ToolbarTool::Pen, cx);
        })
    });
    cx.run_until_parked();
    assert!(cx.debug_bounds("toolbar-draw-action-ClosePath").is_some());

    cx.update(|_, app| {
        toolbar.update(app, |toolbar, cx| {
            toolbar.set_supported_draw_actions([crate::toolbar::DrawToolbarAction::Deselect], cx);
        })
    });
    cx.run_until_parked();
    for selector in [
        "toolbar-draw-action-ClosePath",
        "toolbar-draw-action-JoinPaths",
        "toolbar-draw-action-SimplifyPath",
    ] {
        assert!(
            cx.debug_bounds(selector).is_none(),
