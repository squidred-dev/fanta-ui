    ...
    fn draw_surfaces(
        ...
        for surface in surfaces {
            ...
            if pixel_format == kCVPixelFormatType_32BGRA {
                let Ok(color_texture) = self.core_video_texture_cache.create_texture_from_image(
                ...
                if y_texture.is_null() || cb_cr_texture.is_null() {
                    return false;
                }

                command_encoder.set_render_pipeline_state(&self.surfaces_pipeline_state);
                command_encoder.set_fragment_texture(SurfaceInputIndex::YTexture as u64, unsafe {
                    Some(metal::TextureRef::from_ptr(y_texture as *mut _))
                });
                command_encoder
                    .set_fragment_texture(SurfaceInputIndex::CbCrTexture as u64, unsafe {
                        Some(metal::TextureRef::from_ptr(cb_cr_texture as *mut _))
                    });
                command_encoder.draw_primitives(metal::MTLPrimitiveType::Triangle, 0, 6);
            } else {
                return false;
