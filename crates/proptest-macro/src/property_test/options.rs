...
use syn::{
    parse::Parse, parse_str, punctuated::Punctuated, spanned::Spanned, Expr, Ident, Lit, LitStr,
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        for MetaNameValue { path, value, .. } in pairs {
            ...
            match path_string.as_deref() {
                None => {
                    errors.push(quote_spanned!(path.span() => compile_error!("unknown argument")))
                }
...
mod tests {
    ...
    fn simple_parse_example() {
        let Options {
            ...
            config,
            proptest_path,
        } = parse_str("config = (), random = 123, proptest_path = \"::foo::bar\"").unwrap();
